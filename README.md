...
* **Пустая комната** живёт ещё 60 секунд — можно успеть вернуться, не потеряв прогресс.

## Архитектура

**Authoritative Server**: клиент — тонкий рендерер, отправляющий интенты; сервер валидирует их,
меняет состояние в памяти процесса и транслирует события всем в комнате. Клиент никогда не решает
...
| Состояние | In-memory (`Map<roomId, RoomState>`), без БД |
| База знаний | `elements.json` / `recipes.json`, read-only |
